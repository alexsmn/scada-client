...
ActionGroup::ActionGroup(ActionManager& action_manager)
    : ActionGroup{action_manager, std::make_shared<ActionGroupState>()} {}

ActionGroup::ActionGroup(ActionManager& action_manager,
                         std::shared_ptr<ActionGroupState> state)
    : action_manager_{action_manager}, state_{std::move(state)} {
  assert(state_);
}

ActionGroup ActionGroup::WithContext(ActionContext context) const {
  ActionGroup group{action_manager_, state_};
  group.SetContext(context);
  return group;
}

bool ActionGroup::Contains(unsigned command_id) const {
  return state_->action_ids_.contains(command_id);
}

Action& ActionGroup::AddAction(Action action) {
  auto& registered_action = action_manager_.AddAction(std::move(action));
  state_->action_ids_.insert(registered_action.command_id());
  return registered_action;
}

void ActionGroup::AddActionId(unsigned command_id) {
  state_->action_ids_.insert(command_id);
}

Action* ActionGroup::FindAction(unsigned command_id) const {
  if (!Contains(command_id)) {
    return nullptr;
  }

  if (!action_manager_.IsActionAvailable(command_id, context_)) {
    return nullptr;
  }
...
bool ActionGroup::IsActionEnabled(unsigned command_id) const {
  return Contains(command_id) &&
         action_manager_.IsActionEnabled(command_id, context_);
}

bool ActionGroup::IsActionChecked(unsigned command_id) const {
  return Contains(command_id) &&
         action_manager_.IsActionChecked(command_id, context_);
}

void ActionGroup::ExecuteAction(unsigned command_id) const {
  if (Contains(command_id)) {
    action_manager_.ExecuteAction(command_id, context_);
  }
}
