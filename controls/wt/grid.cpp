...
void Grid::SetContextMenuHandler(ContextMenuHandler handler) {
  ...
  if (context_menu_handler_) {
    if (!context_menu_connection_.isConnected()) {
      context_menu_connection_ = mouseWentUp().connect(
          [this](const Wt::WModelIndex& index, const Wt::WMouseEvent& event) {
            if (event.button() == Wt::MouseButton::Right) {
              auto* popup_menu = new Wt::WPopupMenu{};
              popup_menu->addItem("Popup");
              popup_menu->addItem("Popup");
              popup_menu->addItem("Popup");
              popup_menu->popup(event);
            }
          });
    }
    ...
    context_menu_connection_.disconnect();
  }
...
/*void Grid::mousePressEvent(Wt::WMouseEvent* event) {
