...
TEST_F(ScreenshotGenerator, CaptureAllWindows) {
  ...
  for (const auto& spec : g_config.screenshots) {
    ...
    if (spec.window_type == "Users" &&
        !GetScreenshotOptions().theme.empty()) {
...
std::vector<std::u16string> FixtureAccountsWith(scada::AccessRight right) {
  ...
  const std::string rights_key = NodeIdToScadaString(sec::UserType_AccessRights);
...
TEST_F(ScreenshotGenerator, RolesEnumerateTheirMembers) {
  ...
  const std::optional<std::vector<RoleMembership>> roles =
      WaitForAwaitable(executor_, ReadRoleMemberships(
