...
// come from the severity single source (aui/severity_colors.h, backlog 0.2) so a
// token change restyles the tiles with everything else.
//
...
// source assigns it (nullopt = no severity colour, render in the neutral token).
struct SeverityTile {
  std::string caption;
  int count = 0;
...
std::vector<SeverityTile> BuildSeverityTiles(const SeverityTileCounts& counts,
