...
void ModusView2::Open(const base::FilePath& path) {
  ...
  if (scheme_) {
    ...
    setWindowTitle(QString::fromStdWString(title_));
...
void ModusView2::paintEvent(QPaintEvent* e) {
...

  {
    ...
    base::win::ScopedBitmap bitmap(::CreateCompatibleBitmap(GetDC(0), width(), height()));
...
void ModusView2::CreateBindings() {
  for (auto& shape : renderer_->shapes()) {
    ...
    if (!binding.empty()) {
      bindings_[shape.get()].reset(
          new ModusBinding2(*this, timed_data_service_, *shape, binding.as_string()));
    }
  }
}
...
void ModusView2::PaintSelection(QPainter& painter, modus::Shape& shape) {
  {
    auto rect = BoundsToView(shape.bounds());
    if (!rect.isEmpty()) {
      rect.adjust(-kSelectionInset, -kSelectionInset, kSelectionInset, kSelectionInset);
...
QRect ModusView2::BoundsToView(const modus::Rect& bounds) const {
  return QRect(floor(bounds.x() * scale_),
