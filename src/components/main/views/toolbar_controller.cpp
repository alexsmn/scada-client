...
ToolbarController::ToolbarController(views::Toolbar& toolbar, CommandHandler& command_handler, ActionManager& action_manager)
    : toolbar_(toolbar),
...
void ToolbarController::UpdateCommands() {
  ...
  for (ActionList::const_iterator i = actions.begin(); i != actions.end(); ++i) {
    ...
    if (action.always_visible() || command_handler_.GetCommandHandler(action.command_id()))
  ...
  for (GroupedActions::iterator i = grouped_commands.begin();
    ...
    for (ActionList::iterator i = commands.begin(); i != commands.end(); ++i) {
      ...
      CommandHandler* command_handler = command_handler_.GetCommandHandler(action->command_id());
      if (!command_handler)
        continue;

      views::Toolbar::Item item(views::Toolbar::BUTTON);
      item.command_id = action->command_id();
      item.text = expand_category ? action->GetShortTitle() : action->GetTitle();
...
void ToolbarController::UpdateCommandStates() {
  for (size_t i = 0; i < toolbar_.items().size(); ++i) {
    ...
    CommandHandler* handler = command_handler_.GetCommandHandler(item.command_id);
    if (!handler) {
