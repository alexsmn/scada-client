...
#include "client/components/limits/limit_dialog.h"
...
CommandHandler* SelectionCommands::GetCommandHandler(unsigned command_id) {
  ...
  switch (command_id) {
    ...
    case ID_OPEN_EVENTS:
      return selection_->multiple() ||
             selection_->GetTimedData().connected() ||
...
void SelectionCommands::ExecuteCommand(unsigned command_id) {
  ...
  if (selection_->multiple()) {
    ...
    OpenView(main_window_, PrepareWindowDefinitionForOpen(items, type, title.c_str()));
    return;
  }
  ...
  switch (command_id) {
    case ID_CHANGE_PASSWORD: {
      ...
      if (IsInstanceOf(node, id::UserType))
        ShowChangePasswordDialog(node, local_events_, profile_, node_management_service_);
      return;
    }
  }
  ...
  if (node_id == scada::NodeId() && selection_->GetTimedData().connected()) {
    ...
    if (type) {
      ...
      OpenView(main_window_, PrepareWindowDefinitionForOpen(formula.c_str(), type));
    }
    return;
  }
  ...
  switch (command_id) {
    ...
    case ID_WRITE:
      ExecuteWriteDialog(main_window_, timed_data_service_, node, false, profile_);
      return;
    case ID_WRITE_MANUAL:
      ExecuteWriteDialog(main_window_, timed_data_service_, node, true, profile_);
      return;
    case ID_UNLOCK_ITEM: {
      auto& local_events = local_events_;
      auto& profile = profile_;
      task_manager_.PostUpdateTask(node_id, {}, {{ id::DataItemType_Locked, false }},
          [&local_events, &profile](const scada::Status& status) {
            ReportRequestResult(L"Снять блокировку", status, local_events, profile);
          });
      return;
    }
    case ID_EDIT_LIMITS:
    ...
    case ID_ACKNOWLEDGE_CURRENT:
      ...
      return;
    case ID_ITEM_PARAMS:
      OpenView(main_window_, PrepareWindowDefinitionForOpen(node, ID_PROPERTY_VIEW));
      return;
    case ID_TABLE_CONFIG:
      OpenView(main_window_, PrepareWindowDefinitionForOpen(node, ID_TABLE_EDITOR));
      return;
    case ID_TRANSMISSION_VIEW:
      OpenView(main_window_, PrepareWindowDefinitionForOpen(node, ID_TRANSMISSION_VIEW));
      return;
    case ID_OPEN_WATCH:
      OpenView(main_window_, PrepareWindowDefinitionForOpen(node, ID_WATCH_VIEW));
      return;
    case ID_OPEN_DEVICE_METRICS:
      if (node) {
  ...
  if (!call_command_id.is_null() && IsInstanceOf(node, id::DataItemType)) {
    DoIOCtrl(node.id(), call_command_id, std::vector<scada::Variant>(), local_events_, profile_, method_service_);
    return;
  }
...
void SelectionCommands::OpenModusView(const scada::NodeId& item_id) {
  auto cached_items = file_cache_.GetList(VIEW_TYPE_MODUS).GetFilesContainingItem(item_id);

  if (cached_items.empty()) {
    ShowMessageBox(dialog_service_, L"Схема для объекта не найдена.", L"Схема", MB_ICONEXCLAMATION);
    return;
  }
