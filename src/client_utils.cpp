...
struct ExpandGroupItemIdsHelper
    : public std::enable_shared_from_this<ExpandGroupItemIdsHelper<Callback>> {
  ExpandGroupItemIdsHelper(scada::ViewService& view_service,
  ...
  ~ExpandGroupItemIdsHelper() { callback_{std::move(item_ids_)}; }
  ...
  bool Traverse(const NodeRef& node) {
    ...
    BrowseNodes(
        ...
        {node.id(), scada::BrowseDirection::Forward, scada::id::Organizes,
         true},
...
void ExpandGroupItemIds(scada::ViewService& view_service,
  ...
  std::make_shared<ExpandGroupItemIdsHelper<ExpandGroupItemIdsCallback>>(
      view_service, node_service, callback)
      ->Traverse(node);
}
...
void DeleteTreeRecordsRecursive(const NodeRef& node,
                                TaskManager& task_manager) {
...
WindowDefinition PrepareWindowDefinitionForOpen(const NodeIdSet& items,
  ...
  for (std::set<scada::NodeId>::const_iterator i = items.begin();
       i != items.end(); ++i) {
