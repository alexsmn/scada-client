...
bool ParseEnumStrings(const std::vector<base::StringPiece>& enum_strings, int& value) {
  // TODO:
  return false;
}
...
const HierachicalPropertyDefinition kObjectInput1PropDef({ &kObjectInput1DevicePropDef, &kObjectInput1ChannelPropDef });

const ChannelPropertyDefinition kObjectInput2DevicePropDef(L"Устройство (Резерв)", true);
const ChannelPropertyDefinition kObjectInput2ChannelPropDef(L"Канал (Резерв)", false);
const HierachicalPropertyDefinition kObjectInput2PropDef({ &kObjectInput2DevicePropDef, &kObjectInput2ChannelPropDef });

const ChannelPropertyDefinition kObjectOutputDevicePropDef(L"Устройство (Управление)", true);
const ChannelPropertyDefinition kObjectOutputChannelPropDef(L"Канал (Управление)", false);
const HierachicalPropertyDefinition kObjectOutputPropDef({ &kObjectOutputDevicePropDef, &kObjectOutputChannelPropDef });
...
PropertyDefs GetTypeProperties(const NodeRef& type_definition) {
  ...
  for (auto supertype = type_definition; supertype; supertype = supertype.supertype()) {
...
PropertyDefinition::PropertyDefinition(ui::TableColumn::Alignment alignment, int width)
...
base::string16 PropertyDefinition::GetText(PropertyContext& context, const NodeRef& node, const scada::NodeId& prop_type_id) const {
  auto value = node[prop_type_id].value();
  return {base::SysNativeMBToWide(value.get_or(std::string{})), false};
}

void PropertyDefinition::SetText(PropertyContext& context, const NodeRef& node, const scada::NodeId& prop_decl_id,
                                 const base::string16& text) const {
  const auto& type_definition = node.type_definition();
  if (type_definition)
    return;

  const auto& property_declaration = type_definition.GetAggregateDeclaration(prop_decl_id);
  if (!property_declaration)
    return;

  scada::Variant value;
  if (!StringToValue(base::SysWideToNativeMB(text).c_str(), property_declaration.data_type().id(), value))
    return;

  context.task_manager_.PostUpdateTask(node.id(), {}, {{ prop_decl_id, std::move(value) }});
}

PropertyEditor PropertyDefinition::GetPropertyEditor(PropertyContext& context, const NodeRef& type_definition,
    const scada::NodeId& prop_decl_id) const {
  auto property_declaration = type_definition.GetAggregateDeclaration(prop_decl_id);
  if (!property_declaration)
...
base::string16 ReferencePropertyDefinition::GetText(PropertyContext& context, const NodeRef& node, const scada::NodeId& prop_type_id) const {
  auto type_definition = node.type_definition();
...
void ReferencePropertyDefinition::SetText(PropertyContext& context, const NodeRef& node, const scada::NodeId& prop_type_id,
    const base::string16& text) const {
  const auto& type_definition = node.type_definition();
  ...
  FindNodeRecursive(context.node_service_, id::RootFolder, base::SysWideToNativeMB(text), reference_type.id(),
      [node, prop_type_id, &node_management_service](NodeRef new_target) {
        const auto& old_target = node.reference(prop_type_id);
        if (old_target == new_target)
          return;

        if (old_target) {
          node_management_service.DeleteReference(prop_type_id, node.id(), old_target.id(),
              [](const scada::Status& status) {});
        }
        if (new_target) {
          node_management_service.AddReference(prop_type_id, node.id(), new_target.id(),
              [](const scada::Status& status) {});
        }
      });*/
}

PropertyEditor ReferencePropertyDefinition::GetPropertyEditor(PropertyContext& context, const NodeRef& type_definition,
  ...
  if (auto& reference_type = type_definition.target(prop_type_id)) {
    ...
    /*auto nodes = SyncFindNodesRecursive(context.node_service_, id::RootFolder, reference_type.id());
    for (auto& node : nodes)
...
base::string16 EnumPropertyDefinition::GetText(PropertyContext& context, const NodeRef& node, const scada::NodeId& prop_decl_id) const {
...
PropertyEditor EnumPropertyDefinition::GetPropertyEditor(PropertyContext& context, const NodeRef& type_definition,
    const scada::NodeId& prop_decl_id) const {
  const auto& property_declaration = type_definition.GetAggregateDeclaration(prop_decl_id);
  if (!property_declaration)
    return PropertyEditor(PropertyEditor::NONE);
...
base::string16 ChannelPropertyDefinition::GetTitle(PropertyContext& context, const NodeRef& property_declaration) const {
  return title_;
}

base::string16 ChannelPropertyDefinition::GetText(PropertyContext& context, const NodeRef& node, const scada::NodeId& prop_type_id) const {
...
void ChannelPropertyDefinition::SetText(PropertyContext& context, const NodeRef& node, const scada::NodeId& prop_type_id,
                                        const base::string16& text) const {
  if (!IsInstanceOf(node, id::DataItemType))
    return;
...
PropertyEditor ChannelPropertyDefinition::GetPropertyEditor(PropertyContext& context, const NodeRef& type_definition,
    const scada::NodeId& prop_type_id) const {
  if (device_) {
    PropertyEditor result(PropertyEditor::DROPDOWN);
    result.choices.emplace_back(kChoiceNone);
    /*auto&& nodes = SyncFindNodesRecursive(context.node_service_, id::Devices, id::DeviceType);
    for (auto& node : nodes)
      result.choices.emplace_back(base::SysNativeMBToWide(node.browse_name()));*/
    return result;
  }
...
PropertyEditor TransportPropertyDefinition::GetPropertyEditor(PropertyContext& context, const NodeRef& type_definition,
    const scada::NodeId& prop_type_id) const {
  return PropertyEditor(PropertyEditor::BUTTON);
}

base::string16 ColorPropertyDefinition::GetText(PropertyContext& context, const NodeRef& node, const scada::NodeId& prop_type_id) const {
  auto value = node[prop_type_id].value();
...
void ColorPropertyDefinition::SetText(PropertyContext& context, const NodeRef& node, const scada::NodeId& prop_type_id,
                                      const base::string16& text) const {
  if (text.empty())
    return;
...
PropertyEditor ColorPropertyDefinition::GetPropertyEditor(PropertyContext& context, const NodeRef& type_definition, const scada::NodeId& prop_type_id) const {
  PropertyEditor result(PropertyEditor::DROPDOWN);
