...
std::string FormatReferenceCell(const scada::QualifiedName& browse_name,
                                const scada::NodeId& prop_type_id) {
  return base::StringPrintf("%s @%s", browse_name.name().c_str(),
                            prop_type_id.ToString().c_str());
}
...
void ScanDeleteNodes(const AddressSpaceSnapshot& address_space,
                     const scada::NodeId& type_definition_id,
                     const std::set<scada::NodeId>& exclude_ids,
                     std::vector<scada::NodeId>& deleted_ids) {
  for (auto& p : address_space.nodes) {
    if (IsInstanceOf(p.second, type_definition_id) &&
        exclude_ids.find(p.first) == exclude_ids.end())
...
void ExportConfigurationData::Export(const base::FilePath& path) {
  ...
  RequestNodes(node_service_, {id::DiscreteItemType, id::AnalogItemType},
               [self, this](const std::vector<scada::Status>& statuses,
                 ...
                 GetTypePids(node_service_, discrete_item_type, true,
                             component_declarations_, reference_types_);
                 GetTypePids(node_service_, analog_item_type, true,
                             component_declarations_, reference_types_);
...
void ExportConfigurationData::WriteHeader() {
  ...
  for (auto& component_declaration : component_declarations_)
    writer_.WriteCell(FormatReferenceCell(component_declaration.browse_name(),
                                          component_declaration.id()));
  for (auto& reference_type : reference_types_)
    writer_.WriteCell(
        FormatReferenceCell(reference_type.browse_name(), reference_type.id()));
}

void ExportConfigurationData::WriteNode(const scada::NodeId& parent_id,
                                        const NodeRef& node) {
  ...
  writer_.WriteCell(type ? FormatReferenceCell(type.browse_name(), type.id())
                         : std::string());
  ...
  for (auto& reference_type : reference_types_) {
    ...
    writer_.WriteCell(
        target ? FormatReferenceCell(target.browse_name(), target.id())
               : std::string());
  }
}
...
void ImportConfiguration(TableReader& reader,
                         const AddressSpaceSnapshot& address_space,
    ...
    while (reader.NextCell(cell)) {
      ...
      if (auto property_declaration =
        ...
        if (!StringToValue(cell.c_str(), property_declaration.data_type().id(),
          ...
          auto prop_type_name =
              prop_type ? prop_type.browse_name().name() : "(Неизвестный)";
          throw std::runtime_error("Невозможно распознать значение ячейки '" +
                                   cell + "' с типом '" + prop_type_name + "'");
        }
        ...
        auto old_referenced_id =
            old_ref_node ? old_ref_node.id() : scada::NodeId{};
  ...
  ScanDeleteNodes(address_space, id::DataItemType, listed_nodes,
                  import_data.delete_nodes);
}

void PrintProps(const NodeRef& type_definition,
                const scada::NodeProperties& props,
                std::ostream& report) {
  for (auto& v : props) {
    const auto& prop = type_definition[v.first];
    report << "  " << prop.browse_name().name() << " = "
           << v.second.get_or(std::string{"(Ошибка)"}) << std::endl;
  }
}

void PrintRefs(const AddressSpaceSnapshot& snapshot,
               const std::vector<ImportData::Reference>& refs,
               std::ostream& report) {
  for (auto& r : refs) {
    auto target_name =
        r.add_target_id.is_null()
            ? "(Нет)"
            : snapshot.GetNode(r.add_target_id).browse_name().name();
...
void ShowImportReport(const ImportData& import_data,
  ...
  report << "Пожалуйста, убедитесь в правильности производимых изменений. Если "
            "перечисленные"
         << std::endl
         << "изменения не соответствуют ожидаемым, ответьте Нет на вопрос, "
            "который появится"
         << std::endl
         << "после закрытия данного окна." << std::endl
         << std::endl
         << "ВНИМАНИЕ: При некорректном использовании данная операция может "
            "привести к"
         << std::endl
  ...
  for (auto& p : import_data.create_nodes) {
    ...
    auto type_name =
        type_definition ? ToString(type_definition.display_name()) : "(Ошибка)";
...
void ImportConfigurationFromExcel(DialogService& dialog_service,
  ...
  struct Importer {
    Importer(DialogService& dialog_service, TaskManager& task_manager)
        : dialog_service{dialog_service}, task_manager{task_manager} {}
  ...
  if (!importer->reader.Init(path, kNodeIdTitle)) {
    ShowMessageBox(dialog_service, L"Не удалось открыть файл.", L"Импорт",
                   MB_ICONSTOP);
    return;
  }
      ...
      [importer](AddressSpaceSnapshot address_space) {
        try {
          ...
          auto message = base::StringPrintf(
              "Ошибка при импорте строки %d, столбца %d: %s.",
              importer->reader.row_index(), importer->reader.cell_index(),
              e.what());
          ShowMessageBox(importer->dialog_service,
                         base::SysNativeMBToWide(message).c_str(), L"Импорт",
                         MB_ICONSTOP);
          return;
        }

        if (importer->import_data.IsEmpty()) {
          ShowMessageBox(importer->dialog_service, L"Изменений не найдено.",
                         L"Импорт", MB_ICONINFORMATION);
          return;
        }

        ShowImportReport(importer->import_data, address_space);

        if (ShowMessageBox(importer->dialog_service, L"Применить изменения?",
                           L"Импорт",
