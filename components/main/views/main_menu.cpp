...
MainMenu::MainMenu(MainWindowViews& main_view, HMENU hmenu, ActionManager& action_manager,
    ...
    : action_manager_(action_manager),
      model_(std::make_unique<MainMenuModel2>(MainMenuModel2Context{main_view, profile, file_cache, favourites})),
      hmenu_(hmenu),
      context_menu_index_(-1) {
  ...
  for (int i = 0; i < menu.GetMenuItemCount(); ++i) {
    ...
    mii.dwTypeData = buf;
    mii.cch = sizeof(buf);
    if (menu.GetMenuItemInfo(i, TRUE, &mii)) {
...
void MainMenu::CreateMenuItems(HMENU hmenu, int& pos, UINT& id,
...
LRESULT MainMenu::OnInitMenuPopup(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
  ...
  if ((::GetSubMenu(hmenu_, index) == menu) && (index == context_menu_index_)) {
    for (int i = menu.GetMenuItemCount() - 1; i >= 0; --i)
      menu.DeleteMenu(i, MF_BYPOSITION);
    //if (main_view_.active_view())
    //  InsertMenuItemCommands(menu, 0, *main_view_.active_view());
    ContextMenuModel model(&model_->main_view_, action_manager_);
...
LRESULT MainMenu::OnMenuCommand(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/) {
  int index = static_cast<int>(wParam);
  HMENU menu = reinterpret_cast<HMENU>(lParam);
  
  // main_view_.main_menu_->OnMenuCommand(index, menu);
  
  return 0;
}

LRESULT MainMenu::OnCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
  LRESULT result = 0;
  bHandled = model_->ProcessWindowMessage(NULL, uMsg, wParam, lParam, result);
  return result;
}

LRESULT MainMenu::OnMenuSelect(UINT /*uMsg*/, WPARAM wParam,
                                 LPARAM lParam, BOOL& /*bHandled*/) {
  int index = LOWORD(wParam);
  HMENU menu = reinterpret_cast<HMENU>(lParam);
  
  // main_view_.main_menu_->OnMenuSelect(index, menu);
