#pragma once

#include <string>
#include <vector>

namespace net {
class TransportString;
}

class TransportDialogModel {
 public:
  explicit TransportDialogModel(net::TransportString& transport_string);

  bool IsSerialPortType(int type_index) const;

  std::vector<std::wstring> type_items;
  int type_index = 0;

  std::wstring network_host;
  int network_port = 0;

  std::vector<std::wstring> serial_port_items;
  int serial_port_index = 0;

  std::vector<std::wstring> baud_rate_items;
  int baud_rate_index = 0;

  std::vector<std::wstring> bit_count_items;
  int bit_count_index = 0;

  std::vector<std::wstring> parity_items;
  int parity_index = 0;

  std::vector<std::wstring> flow_control_items;
  int flow_control_index = 0;

  std::vector<std::wstring> stop_bits_items;
  int stop_bits_index = 0;

  void Save();

 private:
  net::TransportString& transport_string_;
};
