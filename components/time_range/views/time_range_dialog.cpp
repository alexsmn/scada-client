...
bool ShowTimeRangeDialog(DialogService& dialog_service,
  ...
  TimeRangeDialog dialog{context.profile_};
  dialog.time_range = context.time_range_;
  dialog.time_required = context.time_required_;
  if (dialog.DoModal(dialog_service.GetDialogOwningWindow()) != IDOK)
    return false;

  context.time_range_ = dialog.time_range;
  return true;
}
