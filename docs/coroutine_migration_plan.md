...
1. Introduce shared coroutine adapters at the client/core boundary.
2. Migrate internal client workflows with the highest callback/promise nesting.
3. Keep public service contracts and module wiring stable until the migrated
   paths are proven in tests.
...
- Standardize coroutine adapters for all SCADA services used by the client.
...
- Reuse the shared adapter utilities introduced in `core/base` and
  ...
  - coroutine wrappers for SCADA services
...
- New coroutine code can await existing SCADA services without inline callback
  wrappers.
...
- Replace service callback wrapping with shared awaitable adapters.
...
- Prefer coroutine adapters over ad hoc wrapper lambdas.
