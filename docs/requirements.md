...
|---|---|---|---|
| UC-1 | Operator | Monitor live values from devices on the configured object tree, with periodic refresh and quality/limit indicators. | `modules/watch/`, `modules/timed_data/` |
| UC-2 | Operator | Visualise multiple time-series on a single chart with configurable time range, panes, colours, dots/steps, and legends. | `modules/graph/`, `modules/timed_data/` |
| UC-3 | Operator | View tabular real-time and historical data with sorting, filtering, and limit highlighting. | `modules/table/`, `modules/sheet/`, `modules/summary/` |
| UC-4 | Operator | Receive and acknowledge events / alarms, with auto-flash, sound, and severity-based colour. | `modules/events/`, `modules/watch/` |
| UC-5 | Operator | Browse historical event journals over a chosen time range, filter by severity / item / area. | `modules/events/event_module.cpp` (`EventJournal` window) |
| UC-6 | Operator | Watch a custom user-defined spreadsheet of cells bound to live values. | `modules/sheet/` |
| UC-7 | Operator | Issue control commands (write set-points, switch states) with optional two-stage confirmation. | `modules/write/` |
| UC-8 | Operator | Track favourite nodes for fast navigation, organise them into portfolios. | `modules/favorites/`, `modules/portfolio/` |
| UC-9 | Operator | Print or export the contents of the active view (table, graph, summary, journal). | `modules/print/`, `modules/export/csv/` |
| UC-10 | Operator | Locate, transfer and view files exchanged with the server (file system view, file cache). | `modules/filesystem/`, `modules/web/` |
| UC-11 | Operator | View Modus 6.30 schematics with click-to-navigate hot-spots overlaid with live values. *(Qt / Windows only)* | `modules/modus/`, `modules/vidicon/display/` |
| UC-12 | Engineer | Browse the device hardware tree, edit per-device parameters, set limits and aliases. | `modules/configuration/devices/`, `modules/limits/`, `modules/node_properties/` |
| UC-13 | Engineer | Create and delete data items in bulk via multi-create dialogs and the table editor. | `modules/multi_create/`, `modules/node_table/` |
| UC-14 | Engineer | Export a configuration snapshot to disk, edit it externally, and import it back. | `modules/export/configuration/` |
| UC-15 | Engineer | Inspect raw protocol traffic for a connected device (request / response inspector). | `modules/debugger/`, `modules/device_metrics/` |
| UC-16 | Engineer | Save the current window layout as a personal profile and restore it on next launch. | `profile/`, `main_window/` |
| UC-17 | Administrator | Authenticate with username / password against a chosen back-end (Scada, OPC UA, Vidicon). | `modules/login/`, `app/client_application.cpp` |
| UC-18 | Administrator | Manage users, change passwords, set access rights. | `modules/change_password/`, `modules/node_table/` (`Users` window) |
| UC-19 | Administrator | Configure data transmission rules between sources and destinations. | `modules/transmission/` |
...
- **FR-2.** Browse the OPC-UA-style address space, expose hierarchical node trees with lazy fetching and progress reporting. — `modules/configuration/`, `modules/node_service_progress_tracker/`.
- **FR-3.** Read attribute values either on demand or via long-lived subscriptions (monitored items). — `modules/watch/`, `aui/models/`.
- **FR-4.** Read historical samples for any node over a user-chosen time range, with cancellation support. — `modules/timed_data/`, history service via `master_data_services.h`.
- **FR-5.** Write values back to nodes (control commands, set-points, manual entry) with optional two-stage confirmation. — `modules/write/`.
- **FR-6.** Receive event/alarm streams, store them in a journal, allow filtering, and acknowledge. — `modules/events/event_module.cpp`, `modules/events/event_fetcher.h`.
...
- **FR-11.** Bookmark frequently used nodes and group them into named **portfolios**. — `modules/favorites/`, `modules/portfolio/`.
- **FR-12.** Print the active view (table, graph, summary, …) with print preview. — `modules/print/`.
- **FR-13.** Export tabular data to **CSV** and the configuration tree to a **portable file format** that can be re-imported. — `modules/export/csv/`, `modules/export/configuration/`.

### Authentication and access control

- **FR-14.** Prompt the user for credentials at startup (or on disconnect), pass them to the chosen back-end, and store the resulting session. — `modules/login/`, `OnLoginCompleted` in `client_application.cpp`.
- **FR-15.** Support **password change** through the UI. — `modules/change_password/`.
...
- **FR-17.** Render the entire UI in **Russian**. The translation lookup goes through a single `Translate()` shim that uses the empty Qt translation context to match `lupdate`-generated `.ts` files. — `aui/qt/translation_qt.cpp`, `app/qt/client_ru.ts`, `main_window/main_window_ru_qt.ts`.
...
- **FR-19.** Embed **Modus 6.30** schematics via ActiveX, exposing live values on top of static drawings, with click-to-navigate. *Qt + Windows only.* — `modules/modus/libmodus/`, `modules/modus/activex/`.
- **FR-20.** Embed the **Vidicon** display protocol for sites running mixed Telecontrol + Vidicon stacks. *Qt + Windows only.* — `modules/vidicon/display/`, `modules/vidicon/teleclient/`.
...
- **NFR-2. Async-first I/O.** Client async implementations use C++20 coroutine bodies with the shared `AwaitPromise`/`ToPromise` adapters in `core/base/awaitable_promise.h`; public and module-crossing APIs may still return `promise<T>` from `scada-core` for compatibility. New async implementation code should prefer `co_await` sequencing and use `ToPromise(...)` only at legacy compatibility boundaries. Migrated first-pass coverage includes `ClientApplication::Start`/`Login`/`Quit`, the Qt application startup chain and E2E smoke/report helpers, screenshot-generator and Qt test wait helpers, `TaskManagerImpl` task bodies, `FileManagerImpl::DownloadFileFromServer`, `OpenFileCommandImpl::{Execute,OpenFile}` together with `OpenJsonFileAsync` and `AddFileAsync`, `EventView::SelectSeverity`, main-window command/open-view flows, method command status reporting, client UI helper compatibility wrappers, device metrics recursive node collection / compatibility wrappers, callback bridge, and window-definition building, CSV export command and Qt parameter-dialog result flows, graph horizontal-range history reads, clipboard recursive paste/copy helpers plus paste validation, configuration tree drop task actions, lazy child fetch, and object visible-node fetch, node property initial fetch/update, Qt time range and modal dialog result completion, Qt finished-dialog/message-box completion, resource error dialog handling, centralized Wt dialog compatibility boundaries, the properties child-definition and transport edit/dialog result flows, export configuration data-builder and missing-executor compatibility rejections, screenshot capture task-manager fallback rejections, and the outbound OPC UA `opcua::ClientSession`. `client/profile/` is synchronous and currently out of scope for coroutine migration. — used pervasively; see `client_application.cpp` start sequence.
- **NFR-3. Dual UI on a shared model layer.** The application is built twice: once linked against Qt 5 (`*_qt` targets, `app/qt/main.cpp`) and once against Wt (`*_wt` targets, `app/wt/main.cpp`). Models live in module roots and are platform-agnostic; Qt-specific implementations live in `qt/` subdirs, Wt-specific in `wt/`. The `UI_WT` macro guards Modus and Vidicon, which are Qt-only. — `aui/client_module.cmake`.
...
- **NFR-8. Observability built in.** A `Tracer` instance is created in the core module before anything else and threaded through every operation; metrics are reported through OpenTelemetry instruments and OTLP export. — `core/core_module.h`, `metrics/`.
