...
| `tree` | object | Parent → children map. Keys are `"<ns>.<id>"` or bare IDs for ns=1. Loaded into `LocalViewService`; the running `AddressSpaceFetcher` walks it via `Browse` to populate the live address space. **Every `nodes` entry must appear here** — a node with no parent cannot be created, so it exists nowhere in the address space. `PopulateFixtureNodes` used to drop such orphans silently; it now fails the run with the offending ids (four fully-specified nodes, one of them the target of an `events` entry, sat unnoticed that way). Either give the node a parent or delete it from `nodes`. |
