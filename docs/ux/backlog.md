...
| 1.3 | **Command palette** — ✅ *landed (opt-in)*: `Ctrl K` / clicking the command-search field opens a modal palette (`main_window/command_palette_qt.{h,cpp}`) that filters every registered command by title — case- and Cyrillic-fold-insensitive matching, prefix-before-interior ranking, in the Qt-free `command_match.{h,cpp}` (unit-tested) — and runs the selection through the existing handler resolution. Validated on the real widget (generator `command-palette` dialog capture). *Remaining:* extend the corpus beyond commands to views and address-space tags; wire the field's live typing straight into the palette. | `core/`, `controller/` | 1.2 | Fuzzy search finds commands (views/tags next); Enter activates. |
