...
| 1.3a | **Palette address-space tags**: search data-item tags (nodes) in the palette and activate one (select/reveal/open). Needs `NodeService` access in `MainWindowContext` (not currently threaded to the main window) and async search-as-you-type wired into the modal palette. | `main_window/`, `common/node_service/`, `core/` | 1.3 | Typing finds tags by name; Enter selects/opens the node. |
